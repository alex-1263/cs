...
struct xs // 定义一个结构体，用来存储选手的信息
{
    int id, grade, power; // id为编号，grade为排名，power为实力
} list1[200000005];

bool cmp(xs s1, xs s2) // 定义一个比较函数，用来排序
{
    if (s1.grade != s2.grade) // 如果排名不同，按排名排序
    {
...
int main() // 主函数
{
    ...
    for (int i = 0; i < n * 2; i++) // 输入选手的实力
    {
        cin >> list1[i].grade; // 输入选手的实力
        list1[i].id = i + 1;   // 给选手编号
    }
    for (int i = 0; i < n * 2; i++) // 输入选手的实力
    {
        cin >> list1[i].power; // 输入选手的实力
    }
    sort(list1, list1 + 2 * n, cmp); // 按照实力排序
    for (int i = 0; i < r; i++)      // 进行r轮比赛
    {
        for (int j = 0; j < n * 2; j += 2) // 进行一轮比赛
        {
